...
defmodule GoogleApi.DLP.V2.Model.GooglePrivacyDlpV2QuasiId do
  ...
  - customTag (String.t): A column can be tagged with a custom tag. In this case, the user must
  indicate an auxiliary table that contains statistical information on
  the possible values of this column (below). Defaults to `nil`.
  - field (GoogleApi.DLP.V2.Model.GooglePrivacyDlpV2FieldId.t): Identifies the column. [required] Defaults to `nil`.
  - inferred (GoogleApi.DLP.V2.Model.GoogleProtobufEmpty.t): If no semantic tag is indicated, we infer the statistical model from
  the distribution of values in the input data Defaults to `nil`.
  - infoType (GoogleApi.DLP.V2.Model.GooglePrivacyDlpV2InfoType.t): A column can be tagged with a InfoType to use the relevant public
  dataset as a statistical model of population, if available. We
  currently support US ZIP codes, region codes, ages and genders.
  To programmatically obtain the list of supported InfoTypes, use
  ListInfoTypes with the supported_by=RISK_ANALYSIS filter. Defaults to `nil`.
  """
